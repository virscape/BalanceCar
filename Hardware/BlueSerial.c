 ...
 ***************************************************************************************
 * @file    BlueSerial.c
 * @brief   蓝牙串口驱动（USART2, 9600-8N1）
 *         PA2→TX, PA3→RX
 *         接收使用数据包格式：[tag,param1,param2,...]
 *         用于蓝牙App调参和虚拟摇杆控制
 ***************************************************************************************
 */
...
char BlueSerial_RxPacket[100];          /* 接收数据包缓冲区（不含[]） */
uint8_t BlueSerial_RxFlag;              /* 接收完成标志 */

/**
 * @brief  蓝牙串口初始化（USART2, 9600-8N1，接收中断）
 */
